...
The `firmware/BME280_srv/BME280_srv.ino` file contains an Arduino sketch for an ESP8266-based sensor device.

*   **Functionality:** The firmware reads temperature, humidity, and pressure data from a connected BME280 sensor.
*   **Configuration:** It uses the **WiFiManager** library to create a web portal for initial setup. This allows a user to connect to the device's Wi-Fi access point and configure credentials for the local Wi-Fi network and the MQTT broker without hardcoding them in the firmware. These settings are saved in the ESP8266's EEPROM.
*   **Data Transmission:** Once configured, the device connects to the MQTT broker and publishes sensor readings every 10 seconds to the `sensors/{device-id}/bme280` topic in JSON format.
*   **Device Status:** It also publishes a retained `online` message to the `devices/{device-id}/status` topic and sets a Last Will and Testament (LWT) to publish an `offline` message if it disconnects, allowing the system to track device availability.
